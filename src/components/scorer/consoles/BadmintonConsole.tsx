    ...
    <div className="rounded-2xl border border-slate-800 bg-slate-900/90 p-5 space-y-6 shadow-xl">
      <div className="flex items-center justify-between border-b border-slate-800 pb-3">
        <h3 className="text-lg font-bold text-teal-400 flex items-center gap-2">
          <span>🏸</span> Badminton Scorer Console
        </h3>
        <select
          value={currentTeamId}
          onChange={(e) => setTeamId(e.target.value)}
          className="rounded-xl border border-slate-700 bg-slate-950 px-3 py-1.5 text-xs font-bold text-white focus:outline-none focus:ring-2 focus:ring-teal-500"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <button
          ...
          disabled={loading}
          className="rounded-xl bg-teal-600 hover:bg-teal-500 text-white font-extrabold py-3 text-sm transition-all shadow-md active:scale-95 disabled:opacity-50"
        >
          🏸 Rally Point (+1)
        </button>
      ...
      <div>
        ...
        <input
          ...
          className="w-full rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-white focus:outline-none focus:ring-2 focus:ring-teal-500"
        />
      </div>
    </div>
  );
